...
async def upload_wallpaper(request: Request, file: UploadFile):
    ...
    # Reject early if Content-Length exceeds the limit (cheap, no read needed)
    content_length = request.headers.get("content-length")
    if content_length and int(content_length) > MAX_UPLOAD_BYTES:
        return JSONResponse(
            {"error": f"Image too large ({content_length} bytes). Max: {MAX_UPLOAD_BYTES} bytes (10 MB)"},
            status_code=400,
        )

    # Read at most MAX_UPLOAD_BYTES + 1 so we can detect oversize without
    # pulling an unbounded payload into memory.
