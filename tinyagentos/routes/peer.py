...
from tinyagentos.rate_limit import MovingWindowLimiter, retry_after_headers
...
# In-memory per-contact rate limiter.  Bounded and monotonic-clocked by the
# shared limiter (tinyagentos.rate_limit).
# FIXME(post-MVP): back with a shared store (Redis or sqlite contacts.db) for
# accurate cross-worker limits.  The shared limiter keeps memory bounded but
# does not coordinate across processes.
_limiter = MovingWindowLimiter(_RATE_MAX_PER_WINDOW, _RATE_WINDOW_SECS)
# contact_id -> the request timestamps still inside its window.  Aliased here
# so tests and an operator can reset a window; the limiter mutates it in place.
_rate_hits = _limiter.hits
...
async def peer_inbox(body: PeerEnvelope, request: Request):
    ...
    # Rate limit
    if not _limiter.check(contact_id):
        raise HTTPException(
            status_code=429,
    ...
    # Mark peer as seen and drain the outbox so queued messages are
    # delivered immediately when the peer becomes active.
