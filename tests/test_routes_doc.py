"""CI guard: the compiled docs/routes.md must match the source library.

A contributor who edits docs/routes.d/ but forgets to rebuild will fail here.
"""
...
MUST_CONTAIN = [
    ...
    "Agent API surface",
    "Device bearer",
    "Project invite redeem route",
    "OS change-event stream",
    "LoRA Studio routes",
    "decisions come back",
...
def test_routes_doc_build_script_exists():
    assert BUILD_SCRIPT.exists(), f"Build script not found: {BUILD_SCRIPT}"


def test_routes_doc_compiled_output_matches_committed():
    ...
    try:
        ...
        assert fresh == committed, (
            "docs/routes.md is out of sync with docs/routes.d/. "
            "Run: python3 scripts/build-routes-doc.py"
        )
    finally:
        tmp_path.unlink(missing_ok=True)


def test_routes_doc_compiled_size_under_limit():
    content = COMMITTED_OUTPUT.read_text(encoding="utf-8")
    assert len(content) <= MAX_CHARS, (
        f"Compiled routes doc is {len(content)} chars, exceeds {MAX_CHARS} limit. "
        "Trim the source files to keep the prompt injectable on small context windows."
    )


def test_routes_doc_must_have_substrings():
    content = COMMITTED_OUTPUT.read_text(encoding="utf-8")
